...
class target:
    ...
    def get_breachdirectory(self, user_query):
        c.info_news("[" + self.target + "]>[breachdirectory.com]")
...
#            exit(1)
        ...
        try:
            ...
            #            if "title" in result:
            #                self.data.append(("BREACHDR_TITLE", result["title"]))
            #            if "domain" in result:
            #                self.data.append(("BREACHDR_DOMAIN", result["domain"]))
            #            if "email" in result:
            #                self.data.append(("BREACHDR_EMAIL", result["email"]))
            #            if "username" in result:
            #                self.data.append(("BREACHDR_UNAME", result["username"]))
            #            if "ip" in result:
